 ...
 */
const fs = require('fs');
const continents = require('./coutries-continents');
...
const aggregate = (filePath) => {
  const content = fs.readFileSync(filePath, 'utf8'); // read file from local
  ...
  array2.forEach((item, index) => {
    // iterate through each row, to identify continent and perform following
    if (item.length > 1 && index > 0) {
      const country = item[reqindex.get('"Country Name"')];
      conti = continents.get(country.substring(1, country.lastIndexOf('"')));
      const checkconti = check(result, conti); // check if continent already exists in result set
      if (conti) {
  ...
  Keyvalues.forEach((item, index) => {
    subset[item] = {}; // add GDP and Population values to each header
    [subset[item].GDP_2012, subset[item].POPULATION_2012] = [result[index][1], result[index][2]];
  });
  subset = JSON.stringify(subset); // convert to JSON
  fs.writeFileSync('./output/output.json', subset); // write to local folder
};
